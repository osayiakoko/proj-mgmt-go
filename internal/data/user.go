...
func ValidatePasswordPlaintext(v *validator.Validator, password string) {
	...
	v.Check(len(password) >= 8, "password", "must be at least 8 digit long")
	v.Check(len(password) <= 72, "password", "must not be more than 72 digit long")
}

func ValidateUser(v *validator.Validator, user *User) {
	v.Check(user.Name != "", "name", "must be provided")
	v.Check(len(user.Name) <= 500, "name", "must not be more than 500 digit long")
